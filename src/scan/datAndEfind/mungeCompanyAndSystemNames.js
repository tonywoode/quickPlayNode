...
module.exports = systems => {
  ...
  //we match the company too when replacing the system name
  ...
  /* later, we had a problem where mame things its 'Amstrad ZX Spectrum' for some spectrums, yeah maybe, but isnt't it 
   * FAR more important to keep all the spectrum's together under a type? Have a look at some pictures of "Amstrad"
   * spectrums and of course they have Sinclair printed on them - remember we're oddly mutating these premade 'munged
   * vars in the object*/
  const replaceCompanyIfSystem = (oldCompany, newCompany, sysRegex = /ZX Spectrum/) => R.map(
    obj => R.assoc(`mungedCompany`, sysRegex.test(obj.mungedSystem) ? obj.mungedCompany.replace(oldCompany, newCompany) : obj.mungedCompany, obj)
  )
  ...
  const mungedSystems = R.pipe(
    ...
    , compRep(/Acorn .*/, `Acorn`), sysMatchRep(`Acorn`, /BBC|Electron/, `BBC`)
    ...
    , replaceCompanyIfSystem(/Amstrad.*/, 'Sinclair', /ZX Spectrum/) //now this corner case
